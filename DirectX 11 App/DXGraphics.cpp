...
bool DXGraphics::InitialiseClass(HWND hwnd, int w, int h)
{
	...
	//Setup Ambient Lighting		TODO:Move into own class or try to reuse CreateBuffer function in Mesh class
	...
	lightingData.ambientLightingColour = DirectX::XMFLOAT3A(1.f, 1.f, 1.f);
	lightingData.ambientLightingStrength = 1.f;
	return true;
}
...
void DXGraphics::RenderFrame(Camera* const camera, const float dt)
{
	...
	HRESULT hr = pDeviceContext->Map(pLightingBuffer.Get(), NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mapRes);
	...
	pDeviceContext->Unmap(pLightingBuffer.Get(), NULL);
	///Unmap(IN, IN)
	...
	pDeviceContext->PSSetConstantBuffers(0, 1, pLightingBuffer.GetAddressOf());
